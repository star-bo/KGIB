...
def get_MKG(file_path):
    """
    Load the knowledge graph from a file.
    :param file_path: Path to the knowledge graph file.
    :return triples: List of triples in the knowledge graph.
    """
    triples = []
    entities = set()
    relations = set()
...
def random_walk_subgraph(smiles_entity, entity_embeddings, relation_embeddings, triples, num_walks=10, walk_length=80):
    # Find the entity index corresponding to the SMILES entity
    entity_index = None
    for idx, entity in enumerate(triples):
        if entity == smiles_entity:
            entity_index = idx
            break
    
    if entity_index is None:
        raise ValueError(f"Entity {smiles_entity} not found in the triples.")
    
    # Initialize lists to store edges and nodes of the subgraph
    edge_index_list = []
    edge_type_list = []
    node_features = []
    
    # Random walk to sample subgraph nodes
    for _ in range(num_walks):
        current_node = entity_index
        for _ in range(walk_length):
            # Get neighboring entities
            neighbors = [triple[2] for triple in triples if triple[0] == current_node] + \
                        [triple[0] for triple in triples if triple[2] == current_node]
            
            if len(neighbors) == 0:
                break
            
            # Randomly choose a neighboring entity to walk to
            next_node = random.choice(neighbors)
            edge_index_list.append([current_node, next_node])
            edge_type_list.append(0)  # Assuming all relations are of type 0
            node_features.append(entity_embeddings[next_node])
            current_node = next_node
    
    # Convert lists to PyG tensors
...
def convert_MKG_to_dataset(MKG_path):
    with open(MKG_path, 'r') as f:
    ...
    entity2id = {entity: str(index) for index, entity in enumerate(entities)}
    relation2id = {relation: str(index) for index, relation in enumerate(relations)}

    output_folder = os.path.dirname(MKG_path)

    entity2id_path = os.path.join(output_folder, 'entity2id.txt')
    with open(entity2id_path, 'w') as f:
        f.write(str(len(entities)) + '\n')
        for entity, entity_id in entity2id.items():
            f.write(entity + '\t' + entity_id + '\n')

    relation2id_path = os.path.join(output_folder, 'relation2id.txt')
    with open(relation2id_path, 'w') as f:
        f.write(str(len(relations)) + '\n')
        for relation, relation_id in relation2id.items():
            f.write(relation + '\t' + relation_id + '\n')

    train2id_path = os.path.join(output_folder, 'train2id.txt')
    with open(train2id_path, 'w') as f:
        f.write(str(len(triples)) + '\n')
        for triple in triples:
            e1_id = entity2id[triple[0]]
            e2_id = entity2id[triple[2]]
            rel_id = relation2id[triple[1]]
            f.write('\t'.join([e1_id, e2_id, rel_id]) + '\n')

    print("MKG Conversion completed.")
