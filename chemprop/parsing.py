...
def add_train_args(parser: ArgumentParser):
    ...
    parser.add_argument('--kge_training', action='store_true', default=False,
                        help='Whether to train the knowledge graph — typically, we perform training when handling a new dataset for the first time.')
    parser.add_argument("--num_steps",  type=int, default= 100, help="sample steps")
    parser.add_argument("--dim_e",  type=int, default= 300, help="kg embedding")
    parser.add_argument("--first_dense_neurons", type=int, default=128, help="ffn1") 
    parser.add_argument("--second_dense_neurons", type=int, default=64, help="ffn2")
    parser.add_argument("--third_dense_neurons", type=int, default=2, help="ffn3")
    parser.add_argument("--mi_weight", type=float, default= 0.001, help="mi")  
    parser.add_argument("--cl_weight", type=float, default= 0.1, help="cl") 
    parser.add_argument("--tau", type=float, default=0.1, help="cl_tau") 
    parser.add_argument("--gumbeltau", type=float, default=2, help="Gumbeltau")
    parser.add_argument("--kge", type=str, default='TransD',choices=['TransD','TransR','TransH','SimplE','ComplEx'], help="kg encoder")
    ...
    parser.add_argument('--save_plt_dir', type=str, default='./ckpt',
                        help='Directory where model checkpoints will be saved')
    ...
    parser.add_argument('--metric', type=str, default=None,
                        choices=['auc', 'prc-auc', 'rmse', 'mae', 'mse', 'r2', 'accuracy', 'cross_entropy','f1','mcc'],
...
def modify_train_args(args: Namespace):
    ...
    if not ((args.dataset_type == 'classification' and args.metric in ['auc', 'prc-auc', 'accuracy','f1','mcc']) or
            (args.dataset_type == 'regression' and args.metric in ['rmse', 'mae', 'mse', 'r2']) or
            (args.dataset_type == 'multiclass' and args.metric in ['cross_entropy', 'accuracy','f1','mcc'])):
