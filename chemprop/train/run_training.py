...
from chemprop.data.utils import get_class_sizes, get_data, get_task_names, split_data, get_MKG, convert_MKG_to_dataset
from chemprop.models import build_model
from chemprop.nn_utils import param_count, total_param_count
...
# =============================================================================
    ...
    debug('Loading MKG')
    args.task_names = get_task_names(args.data_path)
    MKG_path= f"MKGdata/{args.data_path.split('/')[-1].split('.')[0]}/{args.data_path.split('/')[-1].split('.')[0]}_MKG.txt"   
    mkg_name = args.data_path.split('/')[-1].split('.')[0]
    triples, entities, relations = get_MKG(MKG_path)
    debug(f'Number of triples = {len(triples)}')
    debug(f'Number of entities = {len(entities)}')
    debug(f'Number of relations = {len(relations)}')
    convert_MKG_to_dataset(MKG_path)  
    # #-------------------------------------------------------
    ...
    # Track the overall training time
    ...
    for model_idx in range(args.ensemble_size):
        ...
        debug(f'Number of total parameters = {total_param_count(model):,}')
        ...
        info(f'Training time for model {model_idx}: {training_time:.2f} seconds')
